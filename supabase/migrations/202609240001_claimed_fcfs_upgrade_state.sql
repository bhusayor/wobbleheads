-- A GTD result is an upgrade only when the player had already completed an
-- FCFS claim by submitting both their X post and EVM wallet.
alter table public.whitelist_eligibility
  add column if not exists upgraded_from_claimed_fcfs boolean not null default false;
...
declare
  ...
  v_tier text;
  v_threshold integer;
  v_is_claimed_fcfs_upgrade boolean := false;
begin
  ...
  if v_existing.id is not null then
    v_is_claimed_fcfs_upgrade := v_existing.tier='FCFS'
      and v_existing.status='claimed'
      and v_existing.share_status='submitted'
      and v_existing.share_url is not null
      and v_existing.wallet_address is not null
  ...
  if v_existing.id is null then
    insert into whitelist_eligibility(user_id,run_id,reservation_id,tier,threshold_crossed_at,verified_at,upgraded_from_claimed_fcfs)
    values(p_user_id,p_run_id,v_new_res.id,v_tier,v_new_res.threshold_crossed_at,clock_timestamp(),false);
  else
    update whitelist_eligibility set run_id=p_run_id,reservation_id=v_new_res.id,tier=v_tier,
      ...
      upgraded_from_claimed_fcfs=v_is_claimed_fcfs_upgrade
