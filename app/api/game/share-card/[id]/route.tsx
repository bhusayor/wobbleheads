...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: previousFcfs } = data.tier === 'GTD' ? await admin.from('reward_reservations').select('id,game_runs!inner(status,validated_survival_time)').eq('user_id', data.user_id).eq('tier', 'FCFS').eq('status', 'released').neq('run_id', data.run_id).eq('game_runs.status', 'completed').gte('game_runs.validated_survival_time', 45).limit(1).maybeSingle() : { data: null };
