...
export async function POST(request: Request) {
  ...
  if (!validWallet(wallet)) return gameError('Enter a valid EVM wallet beginning with 0x.');
  const { data, error } = await context.admin.from('whitelist_eligibility').update({ wallet_address: wallet.toLowerCase(), claimed_at: new Date().toISOString(), status: 'claimed' }).eq('user_id', context.user.id).eq('status', 'verified').eq('share_status', 'submitted').select('tier,wallet_address').maybeSingle();
  if (error?.code === '23505') return gameError('That wallet is already registered.', 409);
  if (error || !data) return gameError('Share your verified result on X before submitting a wallet.', 409);
